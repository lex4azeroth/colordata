...
function getIWOMFilterData() {
	var filterData = {
		"department": $('#container').find('div').attr('id'),
...
function init_brand_trends(dateRange) {
	...
	var currentPage = $('#container').find('div').attr('id');
	var notTyreplus = currentPage != "iwom_tyreplus";
	if (notTyreplus) {
...
function init_online_topic_records_tyreplus(dateRange) {
	var subURL = iwomBaseURL + "onlinetopicrecords";
	var tableData = [];
	var filterData = getIWOMFilterData();
